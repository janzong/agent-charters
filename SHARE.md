...
1. **先 V2EX / 知乎**（中文，反馈快、攻击性低，能先把硬伤问出来）
2. 修掉暴露的问题，**再发 HN**（Show HN 只有一次机会，评论质量最高也最狠）
3. 最后 Reddit（r/LocalLLaMA 或 r/ChatGPTCoding）

**时间**：HN 在北京时间 **20:00–23:00**（美东上午）；Reddit 同理。
知乎/V2EX 在**工作日 10:00–12:00 或 20:00–22:00**。
...
## 数据和代码
    ...
    数据集：558 行 × 26 列（parquet / jsonl），代码 MIT，数据 CC-BY-4.0
...
- "不要做什么"排在很前面（66%）——信任是从划线开始的
- 坑/pitfall 只有 14%，是最低的一类，但这类知识恰恰没法从代码里反推
- 有 11 份文件全文只有一句"见 CLAUDE.md"，纯做转发
- 中文只有 5%，低得不太正常
