...
# v0.1.9（2026-09-15）两处**补词**，起因是本仓库自己的 `AGENTS.md` 被自家工具判成 8/9
#   （LIMITATIONS §22 / 实测 §22.1）：
#   (a) 正文禁令家族补 `绝不`——中文里最自然的 "never" 之一，而家族里原本只有
#       严禁/禁止/不允许/请勿/切勿/绝对禁止/严格禁止/不得不。本文件写的是「绝不提交原文全文」，
#       于是那条真禁令**从来没被认出来**（当时的 ✓ 是靠一句解释缺口的话里的"禁止"二字蒙到的）。
#   (b) `overview` 标题词表补 `这是什么`——词表里原本只有 概述/简介/一句话/项目介绍/背景。
#   改动前的实测（`work/audit/rule_gap_impact.py`，数字见 §22.1）：对 v0.5 **标签中性**——
#   516 份覆盖率（boundaries 85.7% / overview 32.2%）与逐份标签、内容模式**零变化**；
#   收益只落在将来采到的中文文件上（v0.6 中文 1046 份：boundaries +6、overview +8，
#   新增的 14 份**逐条读过上下文，14/14 真命中、0 假阳性**，其中 8 份就是字面标题「这是什么」）。
#   标题词 `绝对不能做` 实测收益≈0（v0.6 只 1 份含它）⇒ **不收**。
#   ⚠️ v0.5 数据集的行仍写 `ruleset_v0.1.8`（它是 v0.1.8 产出的；工具与数据版本解耦，D24），
#      **不重算、不替换已发布资产**。
#   ⚠️ 这两条词**不解决** §22 那个"提到即命中"机制（一句*讨论*禁令措辞的话也会被算成一条禁令）——
#      那是另一件事，要单独设计"提到 vs 规定"的区分方式并实测。
RULESET_VERSION = "ruleset_v0.1.9"
...
HEAD_RULES: dict[str, list[str]] = {
    ...
    "overview":    ["overview", "purpose", "about", "introduction", "what is",
                    ...
                    # v0.1.9：补 `这是什么`（§22 的 B 探针）。词表里只有 概述/简介/一句话/
                    # 项目介绍/背景，本仓库自己写的「## 这是什么」整节漏标。CJK 关键词按子串匹配，
                    # 所以 `## 这是什么项目` / `# 这是什么？` 一并覆盖（实测新增 8 份、8/8 是真标题）。
                    "这是什么",
                    "项目定位", "定位", "技术栈", "项目简介",
...
# 只收高精度形式：`必须`太泛（"必须运行测试"是 build_test 不是禁令），`不要`在散文里太多，都不收。
    # v0.1.9：家族里补 `绝不`（§22 的 A 探针）。原先只有 严禁/禁止/不允许/请勿/切勿/绝对禁止/
    # 严格禁止/不得不，于是「绝不提交原文全文」「绝不把密钥写进仓库」这类最自然的写法整条丢。
    "boundaries":  [r"(?:绝不|严禁|禁止|不允许|请勿|切勿|绝对禁止|严格禁止|不得不)",
