...
口径来源与代价（work/external_ref_scan.py；v0.2 起已并入数据集，见 SCHEMA.md）：
  · 第一版用「文本里出现任意 .md 路径」→ 命中 87%（442/507），抽验后判定为假阳性机器
    （README 列表、PR 模板、日志文件名全被算进去）。
  · 收紧为「祈使转引」+「载体专名」后才可用（511 份实测）：
    49% 存在祈使转引（`imperative_route`）、15% 指向知识/记忆载体或规则目录
    （`hard_route`），两者并集 54%（`routes_outward`）。
