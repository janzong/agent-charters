...
| **v0.1.5** | 补 CJK 通道：繁体/日文汉字形标题词（`環境`/`設定`/`検証`/`構成`）+ 中文正文模式（禁令/坑/环境/概览/结构/流程/风格） | 先前正文通道 4 类全是英文正则，CJK 文档只剩"标题词 + 命令类强模式"两条窄路 |
| **v0.1.6** | `split_sections` 跳过围栏代码块内的行；配套：奇数围栏数时最后一个标记不作数、围栏里 ≥3 个 md 标题时不认它是代码块 | 此前**任何** `# 注释` 行都被当标题：129/558 份受影响（最多一份 54 行），`# Install`/`# 3. Build` 变成章节 |
| **v0.1.7** | 撤容器型标题词（`start here`/`quick start`/`quick reference`）；裸 `module`/`usage` 收成具体形态；`dependency` 拓扑义改判 `structure` | 容器词本身不含类别信息（独撑 20 份里仅 1 份真是技术栈）；`usage` 独撑 4 份里 3 份是别的类别 |
| **v0.1.8** | 四块（100 份人工核对落地）：①围栏守卫加语言标记门 ②删 `agent note`/`agent tool`/`be concise`、收 `run`/`behavior` ③补 `DO NOT` 通式/`trap`/`e2e`/复数/`intent` ④12 类共现否决；另加 `is_pointer` 反证闸 | 文件级：①−15 ②−27 ③+119/−2 ④+1/−36，指针 7→2（分母 511→516）。详见 `work/audit/v0.1.8-changelist.md` |
| **v0.1.9** | 补两条词（本仓库自己的 `AGENTS.md` 暴露的两处召回缺口）：正文禁令家族加 `绝不`、`overview` 标题词表加 `这是什么` | 对 v0.5 **标签中性**——558 份逐份标签、内容模式与 516 份覆盖率（`boundaries` 85.7% / `overview` 32.2%）零变化；收益在未来采到的中文（v0.6 中文 1046 份 +6/+8，新增 14 份逐条读过 **14/14 真命中**）。`绝对不能做` 实测收益≈0 ⇒ 不收。见 `LIMITATIONS.md` §22.1 |
...
> 头条结论**在 v0.1.7 时**仍成立：构建测试第一名（85.1% vs 流程 67.9%）。
> 但"85.9%"里约 0.8pp 是假阳性——与 `LIMITATIONS.md §11` 是同一类错误。

### 落地效果（516 基座，v0.1.7 → v0.1.8）

100 份人工核对的结果（四块改动，清单见 `work/audit/v0.1.8-changelist.md`）：

| 改动 | 文件级净效果 |
|---|---|
| ① 围栏守卫加语言标记门 | −15（各掉 1 个假标签） |
| ② 删词与收紧（`agent note`/`agent tool`/`be concise`/裸 `run`/裸 `behavior`） | −27（0 新增） |
| ③ 补词（`DO NOT` 通式 / `trap` / `e2e` / 复数 / `intent`） | **+119 / −2** |
| ④ 收紧（task/requirement/process/要求/tool/warning/style/version/standard/rule/边界…） | **+1 / −36** |
| ⑤ 指针反证闸（短文件自带规则就不算指针） | 分母 511 → **516**（指针 7 → 2） |

**⚠️ 头条结论在 v0.1.8 变了**：`boundaries` 补上正文 `Do not …` 通式后覆盖 **85.7%**，
与 `build_test` **82.8%** 成为并列第一（差距 2.9pp < 通式约 3% 的假阳性幅度）。
详见 `FINDINGS.md` §1 与 `LIMITATIONS.md` §13——**引用头条时请用这两个版本化的说法**。

## 已知问题（v0.5 待改进）

0. **规则法无法处理同义表达**：只有写进关键词的措辞才能命中。
   v0.1.1 的扩充都是"补近义词"，不是"学会理解"——这条路天然有上限。
1. **召回率偏低**：v0.1 抽查 12 份中 3 份有漏标（`overview` 和 `agent_meta` 为主）。
   v0.5 后未标签文件 8 份（1.6%）；100 份人工核对的逐类对照表**已出**
   （`work/audit/v0.5-human-vs-rule.md`）：微平均 precision 80–92% / recall 75–84%，
   最弱是 `gotchas` recall 38%——⚠️ 该数字是 in-sample 上界（见 `LIMITATIONS.md` §16）。
