...
| 口径自披露 | ✅ **2026-09-15 修掉「对不上」那一半**：三个脚本现在**两条口径都打印**（留出集 92%/70% 与 97%/71%；in-sample A 组 90%/75% 与 97%/76%；中文那本早已如此），并把写反的抬头描述改对（此前写着「犹豫单列、不计入」，代码却一直计入；in-sample 的宽松口径更是**只算不打印**）。**未改任何对外数字、规则、脚本默认行为**。⚠️ **仍待人裁定**的只剩「对外主推哪条口径」——在此之前对外沿用 92%/70%（更保守，三站更正评论已按它贴出）。见 §20 |
