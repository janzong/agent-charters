...
- ModelScope 同步（需账号）
- **规则要不要改（D32，新增两处待裁定）**：见 `LIMITATIONS.md` §22 与**影响实测 §22.1（2026-09-15）**
  —— ①`boundaries` 的中文禁令家族缺「绝不」（有「严禁/禁止/不允许/请勿/切勿」）②`overview` 的标题词表缺
  「这是什么」（有「概述/简介/一句话」）。**已实测：这两条对 v0.5 是「标签中性」的**——516 份覆盖率
  （`boundaries` 85.7% / `overview` 32.2%）与逐份标签完全不变，收益只落在将来采到的中文文件上（+14 份＝6+8）。
  §22.1 末尾列了**三个选项（加 A+B / 只加 B / 都不加）**，数字都在那儿，选哪个都不用重测；
  每条词的落地成本、以及 `v0.1.9` 这个号已被「h1 守卫 / 模块名型标题守卫 / 全文兜底门放宽」预占的情况都写清了。
  这两处是本仓库自己的 `AGENTS.md` 被自家工具判成 8/9 的直接原因，**且那个 `boundaries` 是假命中**
  （靠文档里「解释这个缺口」那句话里的「禁止」二字命中，见 §22.1），**最小对照已跑通、一字未改**。
