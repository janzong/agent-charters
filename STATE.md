...
| 规则集 | ✅ **`ruleset_v0.1.9`**（2026-09-15）：相对 v0.1.8 只多两条词——正文禁令家族 `绝不`、`overview` 标题词表 `这是什么`。**对 v0.5 标签中性**（`--verify-v05` 用改完的真实规则重算 558 份，逐份标签与内容模式完全一致，覆盖率仍 85.7% / 32.2%）；收益在未来采到的中文上（v0.6 中文 +6/+8，逐条读过 14/14 真命中）。**v0.5 行仍写 `ruleset_v0.1.8`**，不重算、不替换（D24/D38）｜遗留：正文通道"提到即命中"机制**未解决**（§22）|
...
- ~~**规则要不要改（D32，两处待裁定）**~~ ✅ **2026-09-15 已裁定并落地**：走 §22.1 的**选项 1**
  ...
  ⚠️ **仍未解决**：正文通道"**提到即命中**"（§22）——一句*讨论*禁令措辞的话也算一条禁令。
  这要单独设计"提到 vs 规定"的区分方式并实测，**别再往词表里塞词**（塞词补召回、补不了这个机制）。
...
所以本决定只当"补两条明确缺失的高频说法"，并明确写下**遗留**：正文通道仍是字面匹配，
需要单独设计"提到 vs 规定"的区分方式（`STATE.md` §5 已挂账；v0.1.9 不再碰规则）。
...
「h1 守卫 / 模块名型标题守卫 / 全文兜底门放宽」的挂账预占（§5「第三优先」），那张兜底门审计现在顺延到 **v0.1.10**。
