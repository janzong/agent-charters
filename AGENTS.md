...
- **本文件自己就是漏判的例子**：`compare AGENTS.md` 现在报 **8/9**，缺的 `overview`（标题「这是什么」）
  其实写了——中文自然措辞上的召回缺口（`LIMITATIONS.md` §22）。
  还有一处更值得记的：`boundaries` 本来也漏（本文件写的是「**绝不**」，而规则里只有「禁止」家族），
  **只是因为我写了这句解释、句子里出现了「禁止」二字，它才被认出来**——正文通道是"哪句话里
  出现了某个词"，不是"这句话在说什么"。**所以别为了让它变绿去改措辞**：文档是给智能体看的，
  迁就分类器只会让文档变差，还会把工具的缺陷藏起来。
